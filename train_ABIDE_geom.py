...
from models import GeomServer, GeomClient, mlp_fn
import argparse
...
clients = [GeomClient(mlp_fn(in_dim=num_edge), site_datasets[i]) for i in range(num_sites)]
server = GeomServer(mlp_fn(in_dim=num_edge))
...
def update(round):
    ...
    server.aggregate_barycenter(payloads, gamma=args.gamma, c=args.c)
