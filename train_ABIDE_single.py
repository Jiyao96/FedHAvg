...
parser.add_argument('--gamma', type=float, default=1000, help="gamma scaling for hyperbolic projection")
parser.add_argument('--c', type=float, default=4, help="curvature")
...
sites = ['UCLA'] #, 'UM', 'USM', 'UCLA']
print(sites) 
test_sites = ['NYU', 'UM', 'USM']
print(test_sites)
num_sites = len(sites)
...
site_datasets=[]
for s in sites:
    train_sub = collect_subjects_from_folds(sites_list, [s], np.arange(5))    
...
# parameters
test_sub = collect_subjects_from_folds(sites_list, test_sites, np.arange(5)) 
...
def update(round):
    ...
    # server.aggregate_par(payloads, gamma=args.gamma, c=args.c)   
    server.aggregate(payloads)
    # server.aggregate_geom_median(payloads)  
...
test_site_loader=[]
for s in test_sites:
    persite_test_sub = collect_subjects_from_folds(sites_list, [s], np.arange(5))    
...
def per_site_evaluate():
    for i in range(len(test_sites)):
        ...
        print(f"Test site {test_sites[i]}, ACC {acc:.3g} F1 {f1:.3g} AUC {auc:.3g}")    
